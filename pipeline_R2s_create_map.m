...
if ~isfield(opts,'fittingMode'); options.fittingMode='nonlinear'; end
...
R2s=nan(volTemplate.dim); T2s=nan(volTemplate.dim); S0=nan(volTemplate.dim); RSq=nan(volTemplate.dim); model=nan([volTemplate.dim sum(opts.fit,2)]);
...
for i3=1:size(magnitude,3); for i1=1:size(magnitude,1); for i2=1:size(magnitude,2) %loop through voxels
            ...
            [S0(i1,i2,i3),R2s(i1,i2,i3),T2s(i1,i2,i3),RSq(i1,i2,i3),model(i1,i2,i3,:)] = ...
                fit_R2s(acqPars.TE,magnitude(i1,i2,i3,:),opts.fit,opts.threshold1,opts.threshold2,struct('mode',opts.fittingMode));
