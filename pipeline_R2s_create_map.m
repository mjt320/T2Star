...
if ~isfield(opts,'fit'); opts.fit=ones(1,opts.NEchoes); end %if opts.fit not specified, fit all data
...
R2s=nan(volTemplate.dim); S0=nan(volTemplate.dim); RSq=nan(volTemplate.dim); model=nan([volTemplate.dim size(magnitude,4)]);
